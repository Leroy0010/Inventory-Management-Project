...
const navGroups: NavGroup[] = [
  ...
  },
  {
    label: "Inventory",
    icon: Package2,
    items: [{ to: "/inventory-items", label: "Inventory Items", icon: Package2 }],
  },
  {
    label: "Requests",
    icon: FileText,
...
export function Sidebar({ isSidebarOpen, toggleSidebar }: { isSidebarOpen: boolean; toggleSidebar: () => void }) {
  ...
  const renderNavItem = (item: NavItem) => {
    const hasPermission = !item.permissions || canAccess(item.permissions as []);
  ...
  const renderNavGroup = (group: NavGroup) => {
    const hasPermission = !group.permissions || canAccess(group.permissions as []);
    const isExpanded = expandedGroups.includes(group.label);
    if (!hasPermission) return null;

    const visibleItems = group.items.filter((item) => !item.permissions || canAccess(item.permissions as []));
    ...
    return (
      <Collapsible key={group.label} open={isExpanded} onOpenChange={() => toggleGroup(group.label)}>
        ...
        <CollapsibleContent className="space-y-1 pl-6">{visibleItems.map(renderNavItem)}</CollapsibleContent>
      </Collapsible>
    );
  };
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
          ...
          {/* Reports group */}
          <Collapsible open={expandedGroups.includes("Reports")} onOpenChange={() => toggleGroup("Reports")}>
            <CollapsibleTrigger asChild>
              <Button
                variant="ghost"
              ...
              >
                ...
                <ChevronDown
                  className={cn(
                    "w-4 h-4 transition-all duration-200 group-hover:scale-110",
                    expandedGroups.includes("Reports") && "rotate-180"
                  )}
                />
              </Button>
            </CollapsibleTrigger>
            <CollapsibleContent className="space-y-1 pl-6">{reportsGroup.items.map(renderNavItem)}</CollapsibleContent>
          </Collapsible>
        </nav>
