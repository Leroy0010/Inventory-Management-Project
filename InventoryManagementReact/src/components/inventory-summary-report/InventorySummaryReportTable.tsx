    ...
    <Card className={className}>
      <CardHeader>
        <CardTitle className="flex items-center space-x-2">
          ...
          <span>
            {isQuantityReport ? 'Quantity Summary Report' : 'Value Summary Report'}
          </span>
        </CardTitle>
      </CardHeader>
      <CardContent>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-[120px] text-right">
                  {isQuantityReport ? 'Received' : 'Value Received'}
                </TableHead>
                <TableHead className="w-[120px] text-right">
                  {isQuantityReport ? 'Issued' : 'Value Issued'}
                </TableHead>
                <TableHead className="w-[120px] text-right">
                  {isQuantityReport ? 'Carried Forward' : 'Value Carried Forward'}
                </TableHead>
                  ...
                  <TableRow key={item.inventoryId || index}>
                    ...
                    <TableCell className="text-right font-mono">
                      <div className="flex items-center justify-end space-x-2">
                        <span className={getChangeColor(broughtForward, carriedForward)}>
                          {isQuantityReport 
                            ? formatNumber(carriedForward)
                            : formatCurrency(carriedForward)
                          }
                        </span>
                        {getChangeIcon(broughtForward, carriedForward)}
                      </div>
                    </TableCell>
                    <TableCell className="text-center">
                      <div className="flex items-center justify-center">
                        <Badge 
                          variant={carriedForward > broughtForward ? "default" : 
                                  carriedForward < broughtForward ? "destructive" : "secondary"}
                          className="text-xs"
                        >
                          {carriedForward > broughtForward ? '+' : 
                           carriedForward < broughtForward ? '-' : '='}
                          {isQuantityReport 
                            ? Math.abs(carriedForward - broughtForward).toLocaleString()
                            : Math.abs(carriedForward - broughtForward).toFixed(2)
                          }
                        </Badge>
                      </div>
                    </TableCell>
