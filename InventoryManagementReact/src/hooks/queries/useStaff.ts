...
export const useUpdateStaff = () => {
    ...
    return useMutation({
        ...
        onError: (error: unknown) => {
            ...
            if (validationErrors.length > 0) {
                toast.error(
                    `Failed to update staff member: ${friendlyMessage}`,
                    {
                        description: validationErrors.join(', '),
                    }
                );
...
export const useToggleStaffStatus = () => {
    ...
    return useMutation({
        mutationFn: (request: ToggleStaffStatusRequest) =>
            staffApi.toggleStaffStatus(request),
        onSuccess: (_, { id }) => {
            queryClient.invalidateQueries({ queryKey: staffKeys.lists() });
            queryClient.invalidateQueries({ queryKey: staffKeys.detail(id) });
        ...
        onError: (error: unknown) => {
            ...
            if (validationErrors.length > 0) {
                toast.error(
                    `Failed to update staff status: ${friendlyMessage}`,
                    {
                        description: validationErrors.join(', '),
                    }
                );
...
export const useDeleteStaff = () => {
    ...
    return useMutation({
        ...
        onError: (error: unknown) => {
            ...
            if (validationErrors.length > 0) {
                toast.error(
                    `Failed to deactivate staff member: ${friendlyMessage}`,
                    {
                        description: validationErrors.join(', '),
                    }
                );
