...
export default function TransactionReport() {
  ...
  useEffect(() => {
    ...
    const mockSummary: TransactionSummary = {
      totalTransactions: 4,
      totalInValue: 1250.00,
      totalOutValue: 156.25,
      netValue: 1093.75,
      topItems: [
        { itemName: 'A4 Paper Sheets', totalQuantity: 100, totalValue: 1250.00 },
        { itemName: 'Blue Ballpoint Pens', totalQuantity: 25, totalValue: 31.25 },
        { itemName: 'Stapler', totalQuantity: 5, totalValue: 125.00 }
      ]
    };
  ...
  const filteredTransactions = transactions.filter(transaction => {
    const matchesDate = (!filters.dateFrom || transaction.timestamp >= filters.dateFrom) &&
  ...
  const getTransactionTypeIcon = (type: string) => {
    switch (type) {
      case 'IN': return <TrendingUp className="h-3 w-3" />;
      case 'OUT': return <TrendingDown className="h-3 w-3" />;
      case 'TRANSFER': return <Package className="h-3 w-3" />;
      case 'ADJUSTMENT': return <FileText className="h-3 w-3" />;
      default: return null;
    }
  };
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Total In Value</p>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Total Out Value</p>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Net Value</p>
                ...
                <TableRow key={transaction.id}>
                  ...
                  <TableCell>
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">{transaction.sku}</div>
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="flex items-center space-x-1">
                      ...
                      <span>{transaction.unitPrice.toFixed(2)}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center space-x-1">
                  ...
                  <TableCell>
                    <div className="text-sm">
                      {transaction.fromLocation && (
                        <div>From: {transaction.fromLocation}</div>
                      )}
                      {transaction.toLocation && (
                        <div>To: {transaction.toLocation}</div>
                      )}
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="text-sm">{transaction.performedBy}</div>
