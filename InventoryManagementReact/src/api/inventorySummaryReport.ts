...
export class InventorySummaryReportApi {
   ...
   */
  static exportToCSV(data: InventorySummaryItemDto[], filename?: string): void {
    if (data.length === 0) {
      throw new Error('No data to export');
    }

    // Get headers based on data structure
    const isQuantityReport = data[0].quantityBroughtForward !== undefined;
    const headers = isQuantityReport
      ? ['Inventory ID', 'Inventory Name', 'Unit', 'Brought Forward', 'Received', 'Issued', 'Carried Forward']
      : ['Inventory ID', 'Inventory Name', 'Unit', 'Value Brought Forward', 'Value Received', 'Value Issued', 'Value Carried Forward'];
    ...
    link.setAttribute('download', filename || `inventory-summary-report-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  static calculateSummaryStats(data: InventorySummaryItemDto[]) {
    ...
    if (isQuantityReport) {
      return {
        ...
        totalQuantityBroughtForward: data.reduce((sum, item) => sum + (item.quantityBroughtForward || 0), 0),
        totalQuantityReceived: data.reduce((sum, item) => sum + (item.quantityReceived || 0), 0),
        totalQuantityIssued: data.reduce((sum, item) => sum + (item.quantityIssued || 0), 0),
        totalQuantityCarriedForward: data.reduce((sum, item) => sum + (item.quantityCarriedForward || 0), 0),
      ...
      return {
        ...
        totalValueBroughtForward: data.reduce((sum, item) => sum + (item.valueBroughtForward || 0), 0),
        totalValueReceived: data.reduce((sum, item) => sum + (item.valueReceived || 0), 0),
        totalValueIssued: data.reduce((sum, item) => sum + (item.valueIssued || 0), 0),
        totalValueCarriedForward: data.reduce((sum, item) => sum + (item.valueCarriedForward || 0), 0)
      };
    }
  }
}
