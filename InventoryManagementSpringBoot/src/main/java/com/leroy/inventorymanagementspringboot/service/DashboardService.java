...
public class DashboardService implements DashboardServiceInterface {
    ...
    public AdminDashboardDto getAdminDashboard(UserDetails userDetails) {
        ...
        // Admin dashboard doesn't need recent requests - they have AdminRecentActivity
        // component
    ...
    public StorekeeperDashboardDto getStorekeeperDashboard(UserDetails userDetails) {
        ...
        long lowStockItems = department != null
                ? inventoryItemRepository.countByDepartmentAndQuantityLessThanReorderLevel(department)
                : 0;
        ...
        if (department != null) {
            recentRequests = requestRepository.findRecentRequestsForDepartment(department);
        }
    ...
    public StaffDashboardDto getStaffDashboard(UserDetails userDetails) {
        ...
        var user = userRepository.findByEmail(userDetails.getUsername())
                .orElseThrow(() -> new EntityNotFoundException("User not found"));
        long totalInventoryItems = inventoryItemRepository
                .countByDepartmentAndQuantityNotEqualToZero(user.getOffice().getDepartment());
        long unreadNotifications = notificationRepository.countByUserAndIsReadFalse(user);

        // Get user's cart data
        Cart userCart = cartRepository.findByUser(user).orElse(null);
        List<CartItem> cartItems = userCart != null ? cartItemRepository.findByCartOrderByIdAsc(userCart)
                : new ArrayList<>();
