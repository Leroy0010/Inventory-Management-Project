...
/**
 * Service class responsible for handling all business logic related to
 * inventory requests.
 * This includes creating, approving, rejecting, fulfilling, and retrieving
 * requests.
 * It interacts with various repositories and other services to manage request
 * lifecycle,
 * inventory updates, status history, and notifications.
 */
@Service
@Transactional // Ensures that all methods within this service are executed within a
               // transaction.
public class RequestService implements RequestServiceInterface {
     ...
     *
     * @param requestMapper               Mapper for converting between Request
     *                                    entities and DTOs.
     * @param inventoryItemRepository     Repository for managing InventoryItem
     *                                    entities.
     * @param requestRepository           Repository for managing Request entities.
     * @param requestStatusRepository     Repository for managing RequestStatus
     *                                    entities.
     * @param inventoryIssuanceService    Service for handling inventory issuance
     *                                    logic.
     * @param requestStatusHistoryService Service for managing request status change
     *                                    history.
     * @param cartRepository              Repository for managing Cart entities.
     * @param inventoryBatchRepository    Repository for managing InventoryBatch
     *                                    entities.
    ...
    public RequestService(RequestMapper requestMapper,
            ...
            CartRepository cartRepository, InventoryBatchRepository inventoryBatchRepository,
            NotificationService notificationService, UserRepository userRepository, RoleRepository roleRepository) {
    ...
    /**
     * Creates a new inventory request based on the provided DTO and the
     * authenticated user.
     * This method performs several checks including requester validity, item
     * availability,
     * and insufficient stock before persisting the request. It also handles setting
     * the initial
     * status to "PENDING" and logging the status change. Notifications are sent to
     * the requester.
     *
     * @param requestDto  The data transfer object containing details for the new
     *                    request.
     ...
     * @throws IllegalArgumentException if the request has no items, the "PENDING"
     *                                  status is not found,
     *                                  or if there is insufficient stock for any
     *                                  requested item.
     * @throws RuntimeException         if an inventory item specified in the
    ...
    @Auditable( // Custom annotation for auditing this action.
            action = AuditAction.CREATE, entityClass = Request.class)
    public RequestResponseDto createRequest(CreateRequestDto requestDto, UserDetails userDetails) {
        // Retrieve the requester user from the database based on their email
        // (username).
        User requester = userRepository.findByEmail(userDetails.getUsername())
                .orElseThrow(() -> new EntityNotFoundException("Invalid requester"));
        ...
        for (RequestItemDto itemDto : requestDto.getItems()) {
            ...
            // Notify if the stock of the item is low (logic handled within
            // notificationService).
        ...
        // Convert RequestItemDto objects to RequestItem entities and link them to the
        // request.
        ...
        // Set the collection of request items for the request. Using HashSet for unique
        // items.
        request.setItems(new HashSet<>(requestItems));

        // Don't notify the requester of their own request submission
        // They will only receive notifications when their request is
     ...
     * It updates the request status, sets the approver and approval timestamp if
     * approved,
     * logs the status change, and sends a notification to the original requester.
     *
     * @param dto         The data transfer object containing the request ID and
     *                    approval decision.
     * @param userDetails The Spring Security UserDetails of the authenticated user
     *                    performing the approval/rejection.
     ...
     * @throws IllegalArgumentException if the request is not found or the target
     *                                  status (APPROVED/REJECTED) is not found.
     * @throws IllegalStateException    if the request is not in PENDING status.
     */
    @Override
    @Transactional // Ensures the entire method executes as a single transaction.
    @Auditable( // Custom annotation for auditing this action.
            action = AuditAction.REQUEST_APPROVAL_DECISION, entityClass = Request.class, context = "Request approval or rejection", logBefore = true // Log
                                                                                                                                                     // audit
                                                                                                                                                     // event
                                                                                                                                                     // before
                                                                                                                                                     // method
    ...
    public RequestResponseDto approveOrRejectRequest(ApproveRequestDto dto, UserDetails userDetails) {
        ...
        notificationService
                .notifyWithRequest(
                        ...
                        dto.isApprove() ? NotificationType.REQUEST_APPROVED : NotificationType.REQUEST_REJECTED, // Notification
                                                                                                                 // type.
                        request, timestamp);
     ...
     * It updates the request status to "FULFILLED", processes each request item for
     * issuance,
     * sets the fulfiller and fulfillment timestamp, and sends a notification to the
     * requester.
     *
     * @param dto         The data transfer object containing the request ID and
     *                    fulfillment status.
     ...
     * @throws IllegalArgumentException if the request is not found, the "FULFILLED"
     *                                  status is not found,
    ...
    public RequestResponseDto fulfillRequest(RequestFulfilmentDto dto, UserDetails userDetails) {
        ...
        User fulfiller = userRepository.findByEmail(userDetails.getUsername())
                .orElseThrow(() -> new EntityNotFoundException("User not found"));
        // Retrieve the request by its ID.
     ...
     * clears the cart upon successful submission, sets the request status to
     * "PENDING",
     * and sends notifications to both the requester and the relevant storekeeper.
     *
     * @param userDetails The Spring Security UserDetails of the authenticated user
     *                    whose cart is being submitted.
     * @return A RequestResponseDto representing the newly created request from the
     *         cart.
     * @throws EntityNotFoundException  if the user is not found.
     * @throws IllegalArgumentException if the cart is not found, the cart is empty,
     *                                  the "PENDING" status is not found, or there
    ...
    public RequestResponseDto submitCartAsRequest(UserDetails userDetails) {
        ...
        request.setSubmittedAt(LocalDateTime.now()); // Set submission timestamp.
        ...
        // Attempt to find a storekeeper in the user's department to notify about the
        // new request.
        // User's department is obtained via their office, as staff users don't have a
        // direct department field.
        var storekeeperOptional = userRepository.findByDepartmentAndRole(user.getOffice().getDepartment(),
        ...
        if (storekeeperOptional.isPresent()) {
            ...
            logger.warn("No storekeeper found in department {} to notify for new request.",
                    user.getOffice().getDepartment().getName());
        }
     ...
     * @throws IllegalStateException   if a storekeeper user does not have an
     *                                 associated department.
     */
    @Override
    public List<RequestResponseDto> getUserRequests(UserDetails userDetails) {
        // 1. Fetch the User entity, eagerly loading role, department, and
        // office.department
        User user = userRepository.findByEmail(userDetails.getUsername())
    ...
    /**
     * Retrieves a single request by its ID, performing authorization checks based
     * on the current user's role.
     * A user can view a request if they are the requester, approver, fulfiller, or
     * a storekeeper.
     *
     * @param id          The ID of the request to retrieve.
     * @param userDetails The Spring Security UserDetails of the authenticated user
     *                    attempting to view the request.
     ...
     * @throws IllegalArgumentException if the request with the given ID is not
     *                                  found.
    ...
    public RequestResponseDto getRequestById(Long id, UserDetails userDetails) {
        ...
        // Admin is explicitly excluded from viewing requests based on your
        // clarification
        ...
        // This prevents LazyInitializationException when the DTO mapper tries to access
        // them.
